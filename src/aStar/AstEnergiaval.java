...
public class AstEnergiaval {

	//hasonlokeppen fogunk eljarni az Ast osztalyhoz kepest, csak itt a heurisztikus fuggvenyunk nem tavolsaggal, hanem energiaval fog szamolni
	...
	public AstEnergiaval(Pont[][] matr) {
	...
	public void aStarAlg() {
		...
		updateDetails(i, j, newF, newG, newH);
		queue.add(matr[i][j]);
		...
		while (queue.size() != 0) {
			...
			pont = getMinFromQueue(); // kivalasztjuk a legkisebb f erteku elemet a listabol
			...
			neighborTraverse(i, j, ujI, ujJ); // bejarjuk az osszes szomszedjat
		}
		...
		kimenet.add(matr[xxx][yyy]);
		System.out.println("Min energia: " + energy + ", ennyi lepessel: " + k);
		Collections.reverse(kimenet);
		String ss;

		PrintWriter writer = null;
		try {
			writer = new PrintWriter("kimenetEnergiaval.txt", "UTF-8");
	...
	public void neighbor(int i, int j, int ii, int jj) {
		if (matr[ii][jj].getD() == 0) { // ha nem akadaly
			...
			t.setP1(matr[i][j]);
			t.setP2(matr[ii][jj]);
			newG = t.energ() + matr[i][j].getG(); // tavolsag starttol a szomszedig a jelenlegin keresztul
			...
			if (!(closedL.contains(matr[ii][jj])) || newG < matr[ii][jj].getG()) {
				...
				newH = t.energ(); // jelenlegi csomoponttol a vegpontig megtett kozelito tavolsag
				newF = newG + newH;
